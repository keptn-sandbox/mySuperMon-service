...
SERVICENAME = "mysupermon-service"
...
class Keptn:
    ...
    stop_metrics = {}
    report_metrics = {}
    report_worstexecuted_query_metrics = {}
    report_mostexecuted_query_metrics = {}
    report_tableanalysis_metrics = {}
    dbtype = ""
    report_worstExecuted_payload = []
    report_mostExecuted_payload = []
    report_tableanalysis_payload = []
    stop_payload = []
    report_payload = []
    id_num=0
    ...
    def set_auth(username, password):
        token = f"{TOKEN_AUTH_USERNAME}:{TOKEN_AUTH_PASSWORD}"
        authorization = base64.b64encode(token.encode("utf-8")).decode()
        credentials = {'username': username,
                       'password': password,
                       'grant_type': 'password'}
        headers = {'Content-Type': 'application/x-www-form-urlencoded',
                   'Authorization': f"Basic {authorization}"}
        response = requests.post(f"{MYSUPERMON_ENDPOINT}/oauth/token", data=credentials, headers=headers)
        rs = response.json()
        if response.status_code == 200:
            print("INFO --> Mysupermon authentication successful.")
            Keptn.mysupermon_token = rs['access_token']
